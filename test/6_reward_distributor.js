...
function rawTransaction(senderPublicKey, senderPrivateKey, contractAddress, data, value) {
...
function genRandomKey() {
  ...
  for (var i = 0; i < keylen; i++) text += possible.charAt(Math.floor(Math.random() * possible.length));
  return text;
}
...
contract('RewardDistributor', function(accounts) {
  describe('RewardDistributor contract creation and inspection before testing', function() {
    ...
    it(
      'contract deployer accounts[0]=' +
        accounts[0] +
        ' should have init balance of ' +
        contractCreatorRemainBalance +
        ' TBD tokens by default',
      async function() {
        let registry_instance = null;

        registry_instance = await storage_registry.deployed(erc20tok.address, true, proof_of_stake_balance, {
          from: accounts[1],
        });
        ...
        console.log(
          'RewardDistributor deployed with address ' +
  ...
  describe('RewardDistributor exchanging token with Ether test cases', function() {
    ...
    before(async () => {
      ...
      registry_contract = await storage_registry.deployed(erc20_contract.address, true, proof_of_stake_balance, {
        from: accounts[1],
      });
      ...
      if (publicKeys[0] !== owner || publicKeys[1] !== other) {
        throw new Error(
          'Use `truffle develop` and store the keys in ./test/truffle-keys.js' +
      ...
      assert.equal(
        a0,
        contractCreatorRemainBalance,
        'accounts[0]=' + accounts[0] + ' should have balance ' + contractCreatorRemainBalance,
      );
      assert.equal(
        t0,
        pre_fund_amount,
        'registry contract=' + registry_contract.address + ' has init balance ' + pre_fund_amount.times(2),
      );
    });
    ...
    it('should NOT be able to exchangeToken for non-owner', async function() {
      ...
      logging(
        'RewardDistributor contract address ' + registry_contract.address + ' has init Ether balance ' + before_balance,
      );
      ...
      let result = await tryCatch(
        rawTransaction(notOwner, notOwnerPrivateKey, registry_contract.address, data, value),
        errTypes.revert,
      );
      ...
      logging(
        'RewardDistributor contract address ' + registry_contract.address + ' has new Ether balance ' + after_balance,
      );
    ...
    it('should have the same init balance after revert', async function() {
      ...
      assert.equal(
        a0,
        contractCreatorRemainBalance,
        'accounts[0] should have remaining balance ' + contractCreatorRemainBalance,
      );
  ...
  describe('RewardDistributor ipfs metadata and encrypting IPFS test cases', function() {
    ...
    it('should have pre-funded tokens before serving registry', async function() {
      ...
      assert.equal(
        t0,
        default_init_balance.toNumber(),
        'registry_contract ' +
          registry_contract.address +
          ' contract should still have ' +
    ...
    it('ERC20 contract should have pre-registered the correct reward contract address before serving', async function() {
      ...
      assert.equal(
        ...
        registry_contract.address,
        'erc20 contract should have registry_contract address' + erc20_reg_reward_address,
      );
    });
    ...
    it('should be able to register metadata and an encrypted hash by any user', async function() {
      ...
      assert.equal(
        ipfssha256,
        'c180debe61a9f28ec4aef26734af8f19aed8b5d5c6c30cba87b132eea71f04be',
        'sha256 lib not compatible, expecting sha256 c180debe61a9f28ec4aef26734af8f19aed8b5d5c6c30cba87b132eea71f04be but got ' +
          ipfssha256,
      );
      ...
      assert.equal(
        realKey,
        'abcd1234ABCD12348',
        'generating random number has changed, encryption is not backward compatible!',
      );
      ...
      let reg_successful = await registry_contract.encryptIPFS(
        normalize_ipfsMetadata,
        potential_key,
        key2ndIdx,
        l_rand,
        encryptedIPFSHash,
    ...
    it('should be able to register metadata and a 1G file by another user', async function() {
      ...
      assert.equal(
        ipfssha256,
        '92b73a3c06a93b0a5f8d0974efcae2d414015979f577679ae48f71ddf5ac2d33',
        'sha256 lib not compatible, expecting sha256 92b73a3c06a93b0a5f8d0974efcae2d414015979f577679ae48f71ddf5ac2d33 but got ' +
          ipfssha256,
      );
      // TODO: What is the chances of collision here?
      let key2ndIdx = shuffleString(l_rand + ipfssha256 + sha256coder(potential_key));
    ...
    it('should be able to buy and retrieve the IPFS hash by any user with tokens', async function() {
      ...
      assert.equal(
        decryptIPFSHash,
        'QmSzzutTv2AFN6mtLkPs4tDbzqXrVZ82NV6kutmp68bpYd',
        'Expecting the real IPFS decrypted hash to be QmSzzutTv2AFN6mtLkPs4tDbzqXrVZ82NV6kutmp68bpYd',
      );
      assert.equal(
        results[3].toNumber(),
        190921127,
        'The cost to purchase encryptedIdx c180debe61a9f28ec4aef26734af8f19aed8b5d5c6c30cba87b132eea71f04be should be 190921127',
      );
      ...
      assert.equal(
        uploaderNewBalance,
        uploaderBalance + results[3].toNumber(),
        'expected new balance should be 381842254',
      );
      ...
      assert.equal(
        purchaserNewBalance,
        purchaserBalance - results[3].toNumber(),
        'expected remaining balance should be 4999999809078873',
      );
    }); // end test case
    ...
    it('should NOT be able to set reward exchange rate before it is delegated', async function() {
      ...
      let shall_pass_result = await registry_contract.setRewardExchangeRate(filesize_to_token_ex, {
        from: rewardexchanger,
      });
      ...
      await tryCatch(
        registry_contract.setRewardExchangeRate(filesize_to_token_ex, { from: accounts[3] }),
        errTypes.revert,
      );
      await tryCatch(
        registry_contract.setRewardExchangeRate(filesize_to_token_ex, { from: accounts[2] }),
        errTypes.revert,
      );
      await tryCatch(
        registry_contract.setRewardExchangeRate(filesize_to_token_ex, { from: accounts[0] }),
        errTypes.revert,
      );
      ...
      shall_pass_result = await registry_contract.setRewardExchangeRate(filesize_to_token_ex, {
        from: rewardexchanger,
      });
      logging(shall_pass_result);
      new_ex_rate = (await registry_contract.defaultRewardFileSize.call()).toNumber();
  ...
  describe('RewardDistributor filesize exchange rate update and parallel access for encrypted IPFS test cases', function() {
    ...
    before(async () => {
      ...
      logging('registry contract ' + registry_contract.address + ' has init balance ' + default_init_balance);
      await registry_contract.delegateExchangerAddress(rewardexchanger);
      let shall_pass_result = await registry_contract.setRewardExchangeRate(new_filesize_token_ex, {
        from: rewardexchanger,
      });
      logging(shall_pass_result);
      let new_ex_rate = (await registry_contract.defaultRewardFileSize.call()).toNumber();
    ...
    it('should be able to get reward based on new file size exchange rate register by any user', async function() {
      ...
      assert.equal(
        ipfssha256,
        'b3376010d7cf4505b22bd6c32253e879f2c676c323ef8b482f45524629d71574',
        'sha256 lib not compatible, expecting sha256 b3376010d7cf4505b22bd6c32253e879f2c676c323ef8b482f45524629d71574 but got ' +
          ipfssha256,
      );
      // TODO: What is the chances of collision here?
      let key2ndIdx = shuffleString(l_rand + ipfssha256 + sha256coder(potential_key));
      ...
      logging('new balance for publicKeys[6]=' + notOwner + ' is ' + notOwnerBalanceAfter);
      assert.equal(
        notOwnerBalanceAfter,
        notOwnerBalanceBefore + Math.floor(expect_reward.toNumber()),
        'expected reward should be 866129992',
      );
      let reg_balance = (await erc20_contract.balanceOf.call(registry_contract.address)).toNumber();
      logging('registry contract now has remaining balance ' + reg_balance);
      let new_default_init_balance = default_init_balance.sub(expect_reward);
    ...
    it('should be able to buy and retrieve the IPFS hash after updating exchange rate by any user with tokens', async function() {
      ...
      assert.equal(
        decryptIPFSHash,
        'QmWWkGqjgrw3uu6A6swLiYoBcrmwpGonWrWfueB8HNqbWk',
        'Expecting the real IPFS decrypted hash to be QmWWkGqjgrw3uu6A6swLiYoBcrmwpGonWrWfueB8HNqbWk',
      );
      assert.equal(
        results[3].toNumber(),
        484287738,
        'The cost to purchase encryptedIdx b3376010d7cf4505b22bd6c32253e879f2c676c323ef8b482f45524629d71574 should be 484287738',
      );
      ...
      assert.equal(
        uploaderNewBalance,
        uploaderBalance + results[3].toNumber(),
        'expected new balance should be 1350417730',
      );
      ...
      assert.equal(
        purchaserNewBalance,
        purchaserBalance - results[3].toNumber(),
        'expected remaining balance should be 4999999324791135',
      );
    }); // end test case

    it('should be able to use the new fetchParallelKeyForIPFS API to retrieve pass IPFS hash by the same wallet without tokens', async function() {
      let purchaser = publicKeys[7];
      const purchaserRemainingBalance = 4999999324791135; // this will change if you touch any previous examples
      // The following 2 ipfs hashes were already purchased by publicKeys[7] from previous examples.
      // ipfsMetaDataHash => realIPFSHash after AES decrypt
      let purchaseHistory = [
        {
          ipfsMetadataHash: "QmVuzUF8bsKtb9khL3mEnhkedH2buYveJCgnzzCJjvcsAo",
          ipfsRealHash: "QmSzzutTv2AFN6mtLkPs4tDbzqXrVZ82NV6kutmp68bpYd",
          tokcost: 190921127
        },
        {
          ipfsMetadataHash: "QmanimbVzjsNZRn2Gd41oFa3GksVb8DQ3kYKyCJncWyZir",
          ipfsRealHash: "QmWWkGqjgrw3uu6A6swLiYoBcrmwpGonWrWfueB8HNqbWk",
          tokcost: 484287738
        }
      ];
       ...
       * that broke the tracking of total token rewarded and spent for these accounts.
       */
      assert.equal(purchaserBalance, purchaserRemainingBalance, 'expected existing balance to be ' + purchaserRemainingBalance);
      
      for(let i = 0; i < purchaseHistory.length; i++) {
        let mh = purchaseHistory[i]["ipfsMetadataHash"];
        let rh = purchaseHistory[i]["ipfsRealHash"];
        let tok = purchaseHistory[i]["tokcost"];
        let results = await registry_contract.fetchParallelKeyForIPFS(mh, { from: purchaser });
        logging(
          'fetchParallelKeyForIPFS decrypted 1st_partial_key=' +
            results[0] +
            ' 2nd_partial_key=' +
            results[1] +
            ' encryptedHash=' +
        ...
        assert.equal(
          decryptIPFSHash,
          rh,
          'Expecting the real IPFS decrypted hash to be ' + rh,
        );
        assert.equal(
          results[3].toNumber(),
          tok,
          'The expecting cost for ' + mh + ' should be ' + tok,
        );
        purchaserBalance = (await erc20_contract.balanceOf.call(purchaser)).toNumber();
        assert.equal(purchaserBalance, purchaserRemainingBalance, 'expected balance to be intact and should be ' + purchaserRemainingBalance);
      }
    }); // end test case
